class AppStrings {
  AppStrings._();

  static const String appName = 'TIRTA';
  static const String appTagline = 'Deteksi Dini, Edukasi, Hidup Sehat';
  static const String appDescription = 'Pendeteksi Awal TBC';

  static const String login = 'Masuk';
  static const String register = 'Daftar';
  static const String logout = 'Keluar';
  static const String email = 'Email';
  static const String password = 'Kata Sandi';
  static const String fullName = 'Username';
  static const String confirmPassword = 'Konfirmasi Kata Sandi';
  static const String forgotPassword = 'Lupa Kata Sandi?';
  static const String resetPassword = 'Reset Kata Sandi';
  static const String loginWithGoogle = 'Masuk dengan Google';
  static const String noAccount = 'Belum punya akun? ';
  static const String hasAccount = 'Sudah punya akun? ';
  static const String registerNow = 'Daftar sekarang';
  static const String loginNow = 'Masuk sekarang';

  static const String dashboard = 'Beranda';
  static const String expertSystem = 'Sistem Pakar';
  static const String chatbot = 'Chatbot';
  static const String education = 'Edukasi';
  static const String history = 'Riwayat';
  static const String profile = 'Profil';

  static const String startExamination = 'Mulai Periksa';
  static const String startExaminationDesc =
      'Jawab 15 pertanyaan untuk mengetahui tingkat risiko TBC kamu';
  static const String nextQuestion = 'Selanjutnya';
  static const String previousQuestion = 'Kembali';
  static const String seeResult = 'Lihat Hasil';
  static const String retest = 'Periksa Ulang';
  static const String question = 'Pertanyaan';
  static const String of = 'dari';
  static const String yes = 'Ya';
  static const String no = 'Tidak';

  static const String riskVeryLow = 'Risiko Sangat Rendah';
  static const String riskLow = 'Risiko Rendah';
  static const String riskMedium = 'Risiko Sedang';
  static const String riskHigh = 'Risiko Tinggi';
  static const String riskVeryHigh = 'Risiko Sangat Tinggi';

  static const String detectedSymptoms = 'Gejala Terdeteksi';
  static const String recommendation = 'Rekomendasi';
  static const String examinationResult = 'Hasil Pemeriksaan';
  static const String examinationHistory = 'Riwayat Pemeriksaan';

  static const String medicalDisclaimer =
      'Aplikasi ini adalah alat bantu skrining awal, bukan alat diagnosis medis. '
      'Hasil yang ditampilkan hanya bersifat indikatif berdasarkan gejala yang dilaporkan '
      'secara mandiri oleh pengguna. Selalu konsultasikan hasil ini kepada dokter atau '
      'puskesmas terdekat.';

  static const String chatPlaceholder = 'Ketik pertanyaan tentang TBC...';
  static const String chatSend = 'Kirim';
  static const String chatNewSession = 'Sesi Chat Baru';
  static const String chatError =
      'Maaf, terjadi kesalahan. Silakan coba lagi.';
  static const String chatOffline =
      'Tidak ada koneksi internet. Cek jaringan kamu.';

  static const String quickReplyTBC = 'Apa itu TBC?';
  static const String quickReplySymptoms = 'Gejala TBC';
  static const String quickReplyTransmission = 'Cara penularan';

  static const String searchArticles = 'Cari artikel...';
  static const String allCategories = 'Semua';
  static const String readTime = 'menit baca';
  static const String featuredArticles = 'Artikel Terbaru';

  static const String loading = 'Memuat...';
  static const String errorGeneric = 'Terjadi kesalahan. Coba lagi.';
  static const String errorNetwork = 'Koneksi gagal. Cek internet kamu.';
  static const String emptyHistory = 'Belum ada riwayat pemeriksaan';
  static const String emptyArticles = 'Artikel tidak ditemukan';

  static const String greetingMorning = 'Selamat Pagi';
  static const String greetingAfternoon = 'Selamat Siang';
  static const String greetingEvening = 'Selamat Sore';
  static const String greetingNight = 'Selamat Malam';

  static const String deleteHistory = 'Hapus Riwayat';
  static const String deleteHistoryConfirm =
      'Yakin ingin menghapus riwayat ini?';
  static const String cancel = 'Batal';
  static const String delete = 'Hapus';

  static const String editProfile = 'Edit Profil';
  static const String save = 'Simpan';
  static const String saved = 'Tersimpan';
}
